...
### PM-121A — Registered content-type canonical routing

**Steps:**

1. Use a non-core registered content type that supplies a Joomla `router` callback and has a published frontend menu item.
2. Select an item whose canonical frontend URL contains an alias and menu path rather than a generic `/component/.../<numeric-id>` route.
3. Preview the Newsletter, inspect the item title/read-more link, and follow it.
4. Repeat through a test mail and, where practical, an Automatic Newsletter generated by the Scheduled Task path.

**Expected:** Selected-content links use the originating extension's registered router, preserve the source item's alias/language and resolve through the appropriate frontend menu item. A usable registered router must not be replaced by Punga Mail's generic component/view/ID fallback. If an intentionally broken test router throws or cannot be called, Punga Mail falls back safely without breaking Newsletter generation.
