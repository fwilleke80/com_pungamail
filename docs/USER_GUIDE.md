...
**Component layout → Template override → Newsletter override**

Every override is independent. A blank ordinary field means **Inherit**. Image settings provide explicit **Inherit / Use selected image / No image** choices so a child level can also suppress an image configured by its parent.
...
| Logo image | Select an image with Joomla Media Manager. The logo is rendered inside the Header region and may be used without heading text. |
| Logo width / position | Display width plus whether the logo appears above or below the heading. |
| Header | Background colour/image, heading colour, alignment, padding and logo/heading gap. |
| Browser link bar | Background, link colour, alignment and compact padding. |
| Footer | Background colour/image, text/link colours, alignment, padding, divider, and footer reason text. |
...
**New Subscriber is create-only.** If the selected Joomla account or email address already belongs to an existing Punga Mail subscriber, the editor warns immediately, links to the existing subscriber, and refuses to save the new form. The server enforces the same rule, so a duplicate create request can never silently reactivate, overwrite, or replace an existing subscriber or their Channel memberships. To change an existing subscriber, open that subscriber and use the normal edit/reactivation controls deliberately.

The creation form also accepts an optional external recipient name and published-Channel memberships. Channel selection does not replace the global subscription state.
...
| Email subject | Default subject copied to a newsletter. `{date}` inserts the current website date using the site timezone and localized long-date format. A digest can replace the subject with its own pattern. Recipient placeholders such as `{recipient}` and `{userfield|field-name}` are resolved per recipient. |
| Newsletter body (Markdown) | Reusable message content. Place `{new_content}` exactly where selected website content should appear. `{date}` inserts the current website date using the site timezone and localized long-date format. Use `{recipient}` for the recipient name and `{userfield|field-name}` for published Joomla User Custom Fields. |
...
### Recipient-specific placeholders

Newsletters and Templates can use recipient-specific Joomla User Custom Fields with:

```text
{userfield|field-name}
```

For example, a published Joomla User Custom Field whose **Name** is `mobile-phone` is available as `{userfield|mobile-phone}`. Published User Custom Fields are automatically listed in the Markdown editor's **Insert placeholder** menu, so you do not need to remember their field names. The syntax works in both the email subject and Markdown body. The general `{date}` placeholder also works in both places and is available in the Markdown editor placeholder menu.

For a recipient linked to a Joomla user account, Punga Mail resolves the field value for that account. For external email-only subscribers, unknown field names, or unpublished fields, the placeholder becomes an empty value. Values inserted into HTML mail are escaped, so a Custom Field cannot inject arbitrary HTML through this placeholder. Preview and test messages use the currently logged-in administrator's User Custom Fields.
...
Every layout property can be overridden independently. The editor groups them into **Page**, **Browser link bar**, **Header**, **Content**, **Footer**, and **Advanced CSS**. Templates inherit Component Options; Newsletters inherit the selected Template and ultimately Component Options. Background images use Joomla Media Manager and keep the region background colour as a mail-client fallback.
...
**Preview** renders the template. The editor follows Joomla's familiar main-content plus right-sidebar pattern: reusable subject/body live in **Mail content**, visual and inherited message-layout overrides live in **Layout**, while Reply-To remains in the right sidebar. Templates do not have an artificial Published/Unpublished state. Selected-content item formatting is managed centrally under **Content layouts**, not inside individual Templates or Newsletters. **Save**, **Save & Close**, and **Cancel** behave like standard Joomla editor actions.
...
| Email subject | Subject shown in the recipient's mail client. Required. `{date}` inserts the current website date using the site timezone and localized long-date format. |
| Newsletter body (Markdown) | Your message. `{new_content}` inserts selected website content at that exact position. `{date}` inserts the current website date using the site timezone and localized long-date format. `{recipient}` is personalized with a Joomla display name when available and otherwise the email address. |
...
Range formatters can take the **resolved value of another placeholder** as an argument. They remain completely content-type agnostic: replace the example field names with whatever columns the registered content type exposes. For example:
...
Every Layout field can override the applied Template/Component value independently. Leave ordinary fields blank to inherit; use the explicit image/heading/footer modes where an empty value itself has meaning. Newsletter custom CSS is the last CSS layer.
...
| Subject pattern | Optional replacement for the template subject. Supports `{date}` and `{site_name}`. `{date}` is the generation date in the website timezone and localized long-date format. If blank, the template subject is used. |
