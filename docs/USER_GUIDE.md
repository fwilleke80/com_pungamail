...
| Email subject | Default subject copied to a newsletter. A digest can replace it with a subject pattern. Recipient placeholders such as `{recipient}` and `{userfield|field-name}` are resolved per recipient. |
| Newsletter body (Markdown) | Reusable message content. Place `{new_content}` exactly where selected website content should appear. Use `{recipient}` for the recipient name and `{userfield|field-name}` for published Joomla User Custom Fields. |
...
For example, a published Joomla User Custom Field whose **Name** is `mobile-phone` is available as `{userfield|mobile-phone}`. Published User Custom Fields are automatically listed in the Markdown editor's **Insert placeholder** menu, so you do not need to remember their field names. The syntax works in both the email subject and Markdown body.
