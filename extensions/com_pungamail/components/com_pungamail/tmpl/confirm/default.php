...
<?php else : ?>
	<h1><?php echo Text::_('COM_PUNGAMAIL_CONFIRM_TITLE'); ?></h1>
	<p><?php echo Text::sprintf('COM_PUNGAMAIL_CONFIRM_TEXT', '<strong>' . htmlspecialchars((string) $this->subscriber->email, ENT_QUOTES, 'UTF-8') . '</strong>'); ?></p>
	<form action="<?php echo Route::_('index.php?option=com_pungamail&task=subscription.confirm'); ?>" method="post">
		<input type="hidden" name="token" value="<?php echo htmlspecialchars($token, ENT_QUOTES, 'UTF-8'); ?>">
		<button class="btn btn-primary" type="submit"><?php echo Text::_('COM_PUNGAMAIL_CONFIRM_BUTTON'); ?></button>
