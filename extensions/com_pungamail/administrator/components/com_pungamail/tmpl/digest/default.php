...
$item=$this->item;$siteTimezone=(string)Factory::getApplication()->get('offset','UTC');$next=$item?HTMLHelper::_('date',$item->next_run_at,'Y-m-d H:i:s',$siteTimezone):HTMLHelper::_('date','now','Y-m-d H:i:s',$siteTimezone);$confirmAutoChecked=is_object($item)&&property_exists($item,'confirm_auto_send')?(int)$item->confirm_auto_send===1:(string)($item->generation_mode??'draft')==='auto';$storedRecurrenceUnit=(string)($item->recurrence_unit??'weeks');$recurrenceUnit=in_array($storedRecurrenceUnit,['days','weeks','months'],true)?$storedRecurrenceUnit:'days';$recurrenceValue=$storedRecurrenceUnit==='legacy'?max(1,(int)round(((int)($item->recurrence_minutes??1440))/1440)):max(1,(int)($item->recurrence_value??1));
?>
