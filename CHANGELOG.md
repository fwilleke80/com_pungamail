...
- Real Automatic Newsletter sends are unchanged: the generated Newsletter receives a real ID before queueing, and its frozen sent snapshot therefore contains compact authenticated `pm_track` tokens on tracked internal links.
- Removed the "Privacy-conscious reporting" informational note from the Statistics page.
- Added regression coverage for UTM-without-`pm_track` preview/test behavior and the 0.6.31 no-schema-change migration marker.


## 0.6.30 — 2026-09-23

- Replaced newly generated `pm_track` values with a compact authenticated v2 format. UTM values remain visible in the URL and are covered by the HMAC instead of being duplicated inside the token.
- Compact campaign tokens contain only the token format version, Newsletter ID, link index and a 128-bit truncated HMAC-SHA256 signature. They contain no recipient identity and no encrypted payload.
- Kept full validation compatibility with the long campaign-token format generated by Punga Mail 0.6.27–0.6.29, so links in already-sent newsletters continue to record trusted clicks.
- Updated the campaign system plugin and Statistics click map to validate compact tokens against the visible UTM parameters. Altering any authenticated UTM value invalidates the token.
- Added regression coverage for compact token length, UTM tampering, signature tampering and legacy-token compatibility.
- Added the 0.6.30 no-schema-change migration marker and updated campaign-tracking documentation.
